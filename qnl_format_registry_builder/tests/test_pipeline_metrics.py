...
def test_method_profile_metrics_report_direct_and_effective_counts():
    registry = [
        CanonicalFormat(
            canonical_id="fmt-a",
            preferred_name="A",
    ...
    assert metrics["average_effective_method_profiles_per_format"] == 2.667
    assert metrics["max_direct_method_profiles_per_format"] == 2
    assert metrics["max_effective_method_profiles_per_format"] == 5
